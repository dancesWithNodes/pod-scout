...
"""
Name: pod-scout
Description: Lean RunPod GPU watcher. Network-volume aware.
"""

__author__ = "dancesWithNodes"
__version__ = "1.0"
__license__ = "MIT"
    ...
    runpod.headers.update(
        {
            ...
            "User-Agent": "pod-scout/1.0",
        }
    )
    pushover = requests.Session()
    pushover.headers.update({"User-Agent": "pod-scout/1.0"})

    last_pushover_sent_at = 0.0
    last_any_available: bool | None = None

    try:
        print("pod-scout v1.0 - monitor runpod gpu availability...", flush=True)
